...
export async function PATCH(req, { params }) {
  ...
  try {
    ...
    const updated = await prisma.living_info_activity.update({
    ...
    console.error(`[PATCH /api/living-activity/${id}]`, error);
...
export async function DELETE(req, { params }) {
  ...
  try {
    const deleted = await prisma.living_info_activity.delete({
      where: { id: parseInt(id) },
    });

    return NextResponse.json({ message: "Berhasil dihapus", data: deleted }, { status: 200 });
  } catch (error) {
    console.error(`[DELETE /api/living-activity/${id}]`, error);
